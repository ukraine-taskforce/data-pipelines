...
args = getResolvedOptions(sys.argv, ["JOB_NAME", "bucket", "table_name"])
bucket = args["bucket"]
...
requests = glueContext.create_dynamic_frame.from_options(
    "dynamodb",
    connection_options={
...
result = spark.sql(
    """
        select
            ...
            body.location as city_id,
            categoryId as category_id,
            sum(peopleCount) as requested_amount
        from requests_exploded
        group by to_date(to_timestamp(timestamp / 1000)),
            body.location,
            categoryId
    """
)
